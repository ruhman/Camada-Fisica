...
class enlace(object):
    ...
    def client_sync(self):
        ...
        while not sync1:
            ...
            payload, tipo, ok, _, _, _, _ = self.rx.desfaz_package(data)
            if tipo == 2 and ok:
                sync1 = True
            ...
            if run_time > 5:
                print("Erro: NAO recebeu tipo 2 ----> Reenviando tipo 1")
                self.tx.sendBuffer(sync_package)
                timer = time.time()
        ...
        while not sync2:
            data, _ = self.rx.getNData()
            payload, tipo, ok, _, _, _, _ = self.rx.desfaz_package(
                data)
            if tipo == 40 and ok:
                sync2 = True
    ...
    def server_sync(self):
        ...
        while not sync1:
            data, _ = self.rx.getNData()
            _, tipo, ok, _, _, _, _ = self.rx.desfaz_package(
                data)
            if tipo == 1 and ok:
                sync1 = True
        ...
        while not sync2:
            ...
            run_time = time.time() - timer
            if run_time > 5.0:
                print("Erro: tipo 3 NAO recebido ---> Reenviando tipo 2")
                self.tx.sendBuffer(sync_package2)
                timer = time.time()
        self.tx.sendBuffer(sync_package40)

    def client_transmission(self, payload):
        _ = (0).to_bytes(1, byteorder="big")
        timer = time.time()
        ...
        for i in range(len(lista_pacotes)):
            ...
            while True:
                data, _ = self.rx.getNData()
                payload, tipo, ok, _, n_total, pacote_esperado, _ = self.rx.desfaz_package(
                    data)
                if tipo == 5 and ok:
                    sync1 = True
                    ...
                    self.tx.sendBuffer(sync_package4)
                    print(
                        "Erro: CRC diferente do esperado, reenvia pacote {0}".format(i+1))
    ...
    def server_transmission(self):
        ...
        while True:
            data, _ = self.rx.getNData()
            payload, tipo, ok, n_pacote, total_pacotes, _, encoding = self.rx.desfaz_package(
                data)
            ...
            if tipo == 4 and not encoding:
                ...
                print("Recebeu tipo 4 CRC NAO bate, envia 9")
    ...
    def server_encerramento(self):
        ...
        while not encerra:
            data, _ = self.rx.getNData()
            _, tipo, _, _, _, _, _ = self.rx.desfaz_package(data)
            if tipo == 7:
